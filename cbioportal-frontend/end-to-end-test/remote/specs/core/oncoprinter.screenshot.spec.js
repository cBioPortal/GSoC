...
describe('oncoprinter screenshot tests', () => {
    it('oncoprinter genetic only example data', async () => {
    ...
    it('oncoprinter clinical only example data', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
    ...
    it('oncoprinter heatmap only example data', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterHeatmapExampleData')
        ).waitForExist();
        await clickElement('.oncoprinterHeatmapExampleData');
        await clickElement('.oncoprinterSubmit');
        await waitForOncoprint(TIMEOUT);
    ...
    it('oncoprinter example data', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterGeneticExampleData')
        ).waitForExist();
    ...
    it('oncoprinter example data, annotated by oncokb', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterGeneticExampleData')
        ).waitForExist();
        ...
        await browser.waitUntil(async () => {
            return (
                (await getText('.oncoprint-legend-div')).indexOf(
                    'Inframe Mutation (putative driver)'
                ) > -1
            );
        });

        await setOncoprintMutationsMenuOpen(false);

        const res = await checkOncoprintElement();
        assertScreenShotMatch(res);
    });
    it('oncoprinter clinical example data', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterClinicalExampleData')
        ).waitForExist();
    ...
    it('oncoprinter example data, then set gene order, including all genes', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterGeneticExampleData')
        ).waitForExist();
        ...
        await clickElement('.oncoprinterSubmit');
        await waitForOncoprint(TIMEOUT);
        await getElement('.oncoprinterModifyInput', { timeout: TIMEOUT });
        await clickElement('.oncoprinterModifyInput');
        await waitForElementDisplayed('.oncoprinterGenesInput', {
            timeout: TIMEOUT,
        });
        await setInputText('.oncoprinterGenesInput', 'BRCA1 PTEN TP53 BRCA2');
        await clickElement('.oncoprinterSubmit');
        await waitForOncoprint(TIMEOUT);
    ...
    it('oncoprinter example data, then set gene order, not including all genes', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterGeneticExampleData')
        ).waitForExist();
        ...
        await setInputText('.oncoprinterGenesInput', 'BRCA1 PTEN');
        await clickElement('.oncoprinterSubmit');
        await waitForOncoprint(TIMEOUT);
    ...
    it('oncoprinter example data, then set sample order, including all samples', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterGeneticExampleData')
        ).waitForExist();
        ...
        await getElement('.oncoprinterModifyInput', { timeout: TIMEOUT });
        await clickElement('.oncoprinterModifyInput');
        await waitForElementDisplayed('.oncoprinterSamplesInput', {
            timeout: TIMEOUT,
        });
        ...
        await pasteToElement('.oncoprinterSamplesInput', sampleList);
    ...
    it('msk_impact_2017 query STK11:HOMDEL MUT', async () => {
    ...
    it('oncoprinter example data, then set sample order, not including all samples', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterGeneticExampleData')
        ).waitForExist();
    ...
    it('oncoprinter example data, start by set gene order, then set sample order', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterGeneticExampleData')
        ).waitForExist();
    ...
    it('oncoprinter example data, start by set sample order, then set gene order', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterGeneticExampleData')
        ).waitForExist();
        ...
        await setInputText('.oncoprinterGenesInput', 'BRCA2 TP53 PTEN');
        await clickElement('.oncoprinterSubmit');
        await waitForOncoprint(TIMEOUT);
    ...
    it('oncoprinter example data, dont color by mutation type', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterGeneticExampleData')
        ).waitForExist();
        ...
        await waitForOncoprint(TIMEOUT);
        await setOncoprintMutationsMenuOpen(true);
        await clickElement('input[data-test="ColorByType"]');
        await waitForOncoprint(TIMEOUT);
        await setOncoprintMutationsMenuOpen(false); // get it out of the way for screenshot

        const res = await checkOncoprintElement();
        assertScreenShotMatch(res);
    });

    it('oncoprinter example data, hide VUS', async () => {
        await goToUrlAndSetLocalStorage(`${CBIOPORTAL_URL}/oncoprinter`);
        await (
            await getElement('.oncoprinterGeneticExampleData')
        ).waitForExist();
